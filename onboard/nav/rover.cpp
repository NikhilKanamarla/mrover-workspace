...
{
    //cout << "state is " << static_cast<std::underlying_type<NavState>::type>(mCurrentState) << endl;
...
{
    queue<Waypoint> temp = mPath;

    for (size_t i = 0; i < mPath.size(); i++)
    {
        /////cout << "Path is " << temp.front().odom.latitude_deg << " " << temp.front().odom.longitude_deg;
        temp.pop();
    }
    return mPath;
} // path()
...
{
    //if ((bool) mObstacle.detected) {
    ///cout << "is obstacle detected: " << (bool) mObstacle.detected << " distance to obstacle: " << mObstacle.distance << endl;
   // }
    return mObstacle;
} // obstacle()
...
Rover::Rover(const rapidjson::Document &config, lcm::LCM &lcmObject)
    : mRoverConfig(config), mLcmObject(lcmObject), mDistancePid(config["distancePid"]["kP"].GetDouble(),
...
bool Rover::updateRover(RoverStatus newRoverStatus)
{
    ...
    else
    {
        ...
        if (newRoverStatus.autonState().is_auton)
        {
            ...
            mLongMeterInMinutes = 60 / (EARTH_CIRCUM * cos(degreeToRadian(mRoverStatus.odometry().latitude_deg, mRoverStatus.odometry().latitude_min)) / 360);
            return true;
        }
        return false;
    }
